...
def create_hdf5(hf5_file,start_dt,end_dt, nstats, freq,
                data_title = 'Precipitation Data of respective Aggregation',
                utm = False):
    ...
    hf=tables.open_file(hf5_file,'w',filters=tables.Filters(complevel=6))
...
def del_ids(HDF5, hdf5_out, id_delete, utm = False):
    ...
    for idx in range(n_old):
        ...
        for node in coords:
            hf_new.get_node('/coord',node)[ii] = HDF5.f.get_node('/coord',node)[idx]
...
def clean_stations(HDF5,
    ...
    for idx in range(n_old):
        ...
        #the idx of the surviving station)
            ...
            #iidx_all and iidx_merg  != idx_merge which are ALL idx to be merged
        ...
        if utm:
            ...
            coords = ['x2','y2','x3','y3','x4','y4']
        for node in coords:
            hf_new.getNode('/coord',node)[ii] = HDF5.f.getNode('/coord',node)[idx]
...
def merge_hdf5(hdf5_out, hdf5_files):
    ...
    for ifile in range(nfiles):
        ...
        start_idx = np.where(hf.root.timestamps.isoformat[:] == start_iso)[0][0]
...
def _read_HDF5_structure(HDF5):
    ...
    if iagg_dt.days == 1:
        ...
        #integer (minutes)
        freq = '{:d}Min'.format(iagg_dt.seconds/60) ## iagg_dt.minutes, falls verfuegbar
...
def gk_to_lonlat(gk_x,gk_y,gk_choose):
    ...
    if gk_choose == 'gk2' :
        gk = pyproj.Proj(init="epsg:31466") #wgs84 bounds: 5.87 ,49.10, 7.50, 53.75
    elif gk_choose == 'gk3' :
        gk = pyproj.Proj(init="epsg:31467") #wgs84 bounds: 7.50, 47.27, 10.50, 55.06
    elif gk_choose == 'gk4' :
        gk = pyproj.Proj(init="epsg:31468")#wgs84 bounds: 10.50, 47.27, 13.50, 55.06
...
def get_datetimeseries(syear,eyear,td):
    ...
    end_dt=datetime.datetime(eyear+1,1,1)-datetime.timedelta(0,int(td)*60)
