...
for i in range(10):
    ...
    path_to_dwd_quantile_interpolated_using_dwd_netatmo_season_cdf_daily_data = (
        ...
        r"\interpolated_quantiles_from_qnts_dwd_daily_data_basedon_quantiles_%s_season_using_dwd_netamo_grp_%s.csv"
        % (season, str(i)))
    ...
    path_to_dwd_quantile_interpolated_using_dwd_season_cdf_daily_data = (
        ...
        r"\interpolated_quantiles_from_qnts_dwd_daily_data_basedon_qunatiles_%s_season_using_dwd_only_grp_%s.csv"
        % (season, str(i)))

    path_to_dwd_quantile_interpolated_using_netatmo_season_cdf_daily_data = (
        r"X:\hiwi\ElHachem\Prof_Bardossy\Extremes"
        r"\oridinary_kriging_compare_DWD_Netatmo"
        r"\interpolated_quantiles_from_qnts_dwd_daily_data_basedon_qunatiles_%s_season_using_netatmo_only_grp_%s.csv"
        % (season, str(i)))
    ...
    #                                ppt_vals_season.max()) + 2])
            ...
            plt.savefig(out_plots_path / (
                'oberserved_vs_interpolated_cdf_%s_season_%s_%d.png'
                % (title_add, _dwd_stn_, i)),
