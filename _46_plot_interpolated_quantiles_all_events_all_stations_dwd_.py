...
main_dir = Path(r'X:\hiwi\ElHachem\Prof_Bardossy\Extremes\oridinary_kriging_compare_DWD_Netatmo')
...
for temp_freq in ['60min', '360min', '720min', '1440min']:
    ...
    df_dwd_edf.fillna(0, inplace=True)
    ...
    df_improvements = pd.DataFrame(
        index=df_dwd_edf.columns,
           columns=['pearson_corr_dwd_', 'spearman_corr_dwd_',
                   ...
                   'pearson_corr_dwd_netatmo_unc', 'spearman_corr_dwd_netatmo_unc'])
    ...
    try:
        for df_file in data_to_use:
            ...
            if ('combined_interp_dwd') in df_file and 'netatmo' not in df_file:
                print(df_file)
    ...
    df_dwd_edf = df_dwd_edf.loc[df_dwd_edf.index.intersection(df_netatmo_dwd.index), :]
    ...
    try:
        ...
        for event_date in df_dwd.index.intersection(df_dwd_edf.index):
            ...
            for stn_ in original_quantile.index:
                ...
                edf_stn_interp_netatmo_dwd_unc = interpolated_quantile_netatmo_dwd_unc.loc[stn_]
                if edf_stn_orig > 0:
                    ...
                    netatmo_dwd_interp_vals_unc.append(edf_stn_interp_netatmo_dwd_unc)
            ...
            corr_netatmo_dwd_unc = pears(orig_edf_vals, netatmo_dwd_interp_vals_unc)[0]
            rho_netatmo_dwd_unc = spr(orig_edf_vals, netatmo_dwd_interp_vals_unc)[0]
            
            df_compare.loc[event_date, 'pearson_corr_dwd_'] = corr_dwd
            df_compare.loc[event_date, 'spearman_corr_dwd_'] = rho_dwd
            df_compare.loc[event_date, 'pearson_corr_dwd_netatmo'] = corr_netatmo_dwd
            df_compare.loc[event_date, 'spearman_corr_dwd_netatmo'] = rho_netatmo_dwd           
            
            df_compare.loc[event_date, 'pearson_corr_dwd_netatmo_unc'] = corr_netatmo_dwd_unc
            df_compare.loc[event_date, 'spearman_corr_dwd_netatmo_unc'] = rho_netatmo_dwd_unc
    ...
    ax.set_title('Pearson Correlation Interpolated Quantiles from DWD or DWD-Netatmo\n '
                ...
                % (temp_freq, _interp_acc_,
                  ...
                  stations_with_improvements_unc, df_compare.pearson_corr_dwd_netatmo_unc.shape[0],
    ...
    plt.savefig((path_to_use / (
                r'spatial_pears_corr_%s_events_dwd_%s.png' % (temp_freq, _interp_acc_))),
    ...
    ax.set_title('Spearman Correlation Interpolated Quantiles from DWD or DWD-Netatmo \n '
                ...
                % (temp_freq, _interp_acc_,
                   ...
                   percent_of_improvment,
                  stations_with_improvements_unc, df_compare.spearman_corr_dwd_netatmo_unc.shape[0],
                   percent_of_improvment_unc))
    ax.grid(alpha=0.25)
    ...
    plt.savefig((path_to_use / (
                r'spatial_spearm_corr_%s_events_dwd_%s.png' % (temp_freq, _interp_acc_))),
                frameon=True, papertype='a4', bbox_inches='tight', pad_inches=.2)
    plt.close()
...
path_interpolated_using_dwd_netatmo
